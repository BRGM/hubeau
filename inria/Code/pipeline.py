...
with open("queries.txt", "a") as a_file:
    while True:
        ...
        #query = "Quel est le niveau de la nappe à 03635X0545/PZ1 le 15-03-2020 "
        a_file.write(query)
        a_file.write("\n")
        ####################################################################################################################
        ...
        if len(bss_codes) > 0:
            ...
            if len(dates_to_keep)>0:
                for date in dates_to_keep:
                    data.append(get_table_data_bss(bss_codes, start_date=date.get("start_date", None), end_date=date.get("end_date", None)))
            else:
            ...
            if len(dates_to_keep)>0:
                for date in dates_to_keep:
                    data.append(get_table_data_locations(coms_to_keep, deps_to_keep, regs_to_keep, start_date=dates_to_keep["start_date"], end_date=dates_to_keep["end_date"]))
            else:
