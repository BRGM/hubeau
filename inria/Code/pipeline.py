...
query = "niveau de la nappe parisienne aujourd'hui"
MODEL_PATH = "NER_tool/stacked-standard-flair-150-wikiner.pt"
##################################################################################

communes_ = pd.read_csv("demonyms/Data/commune2021.csv")["LIBELLE"].to_numpy().reshape(-1)
all_communes = np.unique(communes_)
...
if bss == -1:
    ...
    elif len(regions) == 0 and len(departements) > 0 and len(communes) == 0: #simple case where there are only departements
